...
module Badges
  ...
  class AuthorizedTest < Test::Unit::TestCase
    ...
    def test_authorizables_with_privilege
      tu = Badges::TestUser.create(:username =>'tu')
      tp = Badges::TestProject.create(:name =>'tp')

      tu.grant_role('manager', tp)
      assert_equal 1, tu.authorizables.size
      assert_equal 1, tu.authorizables(Badges::TestProject).size
      assert_equal 1, tu.authorizables(Badges::TestProject, 'manager').size
      assert_equal 0, tu.authorizables(Badges::TestProject, 'foo').size
    end
