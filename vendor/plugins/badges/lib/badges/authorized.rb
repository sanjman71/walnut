module Badges
  module Authorized
    ...
    module InstanceMethods
      ...
      def authorizables(klass=nil, privilege=nil)
        authorizables = user_roles.inject([]) do |auths, ur|
          unless (ur.authorizable.nil? || (!klass.nil? && !ur.authorizable.is_a?(klass)))
            auths << ur.authorizable if (  privilege.nil? || 
                ((ur.authorizable.respond_to?(:accepts_privilege?) && ur.authorizable.accepts_privilege?(privilege, self)) ||
                (!ur.authorizable.respond_to?(:accepts_privilege?) && self.has_privilege?(privilege, ur.authorizable))))
          end
          auths
        end
        authorizables.uniq
